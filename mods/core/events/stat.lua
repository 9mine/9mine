local write = function(player, _, fields)
    local player_name = player:get_player_name()
    local file_path = fields["file_path"]
    local content = fields["content"]

    if not file_path or not content then return end
...
local write_rc = function(player, formname, fields)
    ...
    if fields.execute then
        ...
        minetest.show_formspec(player_name, formname, table.concat(
                                   {
                "formspec_version[4]", "size[13,13,false]",
                "field[0,0;0,0;file_path;;" .. file_path .. "]",
                "textarea[0.5,0.5;12.0,5;content;;",
                formname == "stat:edit_rc" and minetest.formspec_escape(content) or
                    "", "]", "textarea[0.5,6.2;12.0,5;;;",
                minetest.formspec_escape(execute_result), "]",
                "button_exit[7,11.6;2.5,0.9;write;write]",

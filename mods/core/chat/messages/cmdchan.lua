        ...
        if string.match(pcmd, cmd) then
            ...
            if response:match("^/") then
                ...
                if node and node.p then
                    ...
                    local result, call_response =
                        pcall(stat_read, addr, response, player_name)
                    if result then
                        if call_response.qid.type == 128 then
                            ...
                            local seconds =
                                list_path(addr, parent_path, player) + 0.5
                            minetest.after(seconds,
                                           function(graph, addr, response,
                                                    player)
                                local node =
                                    graph:findnode(hex(addr .. response))
                                local pp = table.copy(node.p)
                                pp.x = pp.x - 2
                                pp.y = pp.y + 1
                                pp.z = pp.z - 2

...
local function console(player, formname, fields)
    if formname == "core:console" then
        ...
        local response =
            connections:get_connection(player_name, lua_entity.addr)
                :get_cmdchan():execute(fields.input:gsub(
                                           lua_entity.user == "glenda" and "%% " or
                                               "; ", ""))

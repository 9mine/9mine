...
function WriteTool.write(entity, player_name)
    ...
    print(directory_entry.stat.name)
    print(directory_entry.stat.name:match(".rc$"))
    if directory_entry.stat.name:match(".rc$") then 
        minetest.show_formspec(player_name, "stat:write_rc",
        table.concat({"formspec_version[4]", "size[13,13,false]", "field[0,0;0,0;file_path;;" .. directory_entry.path .. "]",
                      "textarea[0.5,0.5;12.0,10.6;content;;]", "button_exit[7,11.6;2.5,0.9;write;write]", "button[10,11.6;2.5,0.9;execute;execute]"}, ""))
    else

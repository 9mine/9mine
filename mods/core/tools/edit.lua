...
function EditTool.edit(entity, player, player_name)
    ...
    if not response then
        ...
        if directory_entry.stat.name:match(".rc$") then
            minetest.show_formspec(player_name, "stat:edit_rc", table.concat(
                                       {
                    ...
                    "button_exit[7,11.6;2.5,0.9;edit;edit]",
                    "button[10,11.6;2.5,0.9;execute;execute]"
                }, ""))
        else
            minetest.show_formspec(player_name, "stat:edit", table.concat(
                                       {
